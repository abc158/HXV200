...
******************************************************************/
U8 time_set_state[IR_DECODER_MAX];
U8 time_set_buf[IR_DECODER_MAX][5];
U8 remote_signal[IR_DECODER_MAX];
                           ...
                           u16 ir_state )
{
 ...
 //   if ((decode_p->timer==decode_p->count)&&(decode_p->count>=10)&&(decode_p->count<=13))
    if (((decode_p->timer>=decode_p->count)&&(decode_p->timer<=decode_p->count+2))&&(decode_p->count>=20)&&(decode_p->count<=26))
    {
        decode_p->count=0;
        decode_p->timer=0;
  ...
  case REMOTE_START_BIT:        //  前导码完成
    ...
    else if(((decode_p->timer>=decode_p->count)&&(decode_p->timer<=decode_p->count+2)) && (decode_p->count>=2) && (decode_p->count<=5))
    {
    ...
    if (decode_p->count > 0)
    {
	      ...
	      if (decode_p->bitsDecoded >= 8)
	      {
                ...
                else if(decode_p->rxByte!=0xDD)
                {
                 ...
                 // if(!decode_p->REMOTE_signal)
                    remoteStatus = 1;
                  else if(decode_p->REMOTE_signal == 1)
                  {
                    remoteStatus = 2;
