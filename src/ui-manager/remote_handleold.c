...
void ir_rx_decode(void)
{
  ...
  for (index=0; index<IR_DECODER_MAX; index++)
  {
    ...
    { 
      ...
      if (value != 0x00)
      { 
        //printf("IR VALUT 0x%x ", value);//luyanjin modif
      }
     for(i=0; i<8; i++)
    //for(i=0; i<16; i++)     
      {	  
        //state = (value >> (15-i))&0x01; 	
 ...
 //           if ((remote_decode[index].rxByte&0xff00)==0xDD00)
 //           {
//            // current_ir_code = remote_decode[index].rxByte;//存放遥控器的数据
//             current_ir_code =(U8)remote_decode[index].rxByte&0xff;
 ...
 //           else
            {
              ...
              ir_val[index] = (U8)remote_decode[index].rxByte&0xff;  //存放充电座的信号
...
void ir_rx_code_get(uint8_t chan, uint8_t val)
{
    ...
    ir_val[chan]=val;
    //printf("current_ir_code ir_rx_code_get=%x\r\n",current_ir_code);
   //if(chan == 1)
   //    local_ir_code = val;
